...
public class BarChartRenderer: ChartDataRendererBase
{
    ...
    internal func prepareBarHighlight(x x: CGFloat, y: Float, barspacehalf: CGFloat, from: Float, trans: ChartTransformer, inout rect: CGRect)
    {
        let barWidth: CGFloat = 0.5;
    ...
    public override func drawHighlighted(context context: CGContext, indices: [ChartHighlight])
    {
        ...
        for (var i = 0; i < indices.count; i++)
        {
            ...
            {
                let e = barData.getDataSetByIndex(dataSetIndex)!.entryForXIndex(index) as! BarChartDataEntry!;
